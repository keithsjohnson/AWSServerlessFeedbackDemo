resource "aws_lambda_function" "feedback-lambda" {
    filename = "../build/feedback-lambda.zip"
    ...
    source_code_hash = "${base64sha256(file("../build/feedback-lambda.zip"))}"
}

resource "aws_lambda_function" "feedback-aggregates-lambda" {
    filename = "../build/feedback-aggregates-lambda.zip"
    ...
    source_code_hash = "${base64sha256(file("../build/feedback-aggregates-lambda.zip"))}"
}

resource "aws_lambda_function" "feedback-aggregates-scan-all-lambda" {
    filename = "../build/feedback-aggregates-scan-all-lambda.zip"
    ...
    source_code_hash = "${base64sha256(file("../build/feedback-aggregates-scan-all-lambda.zip"))}"
}
